...
async def get_drink_by_id( item_id: str):
    cached_item = await get_cache(f"item_{item_id}") 
    if cached_item:
    ...
    set_cache(f"item_{item_id}", drink)
...
async def get_drink_by_category( category: str):
    cached_items = await get_cache(f"category_{category}")  
    if cached_items:
    ...
    set_cache(f"category_{category}", drinks)
